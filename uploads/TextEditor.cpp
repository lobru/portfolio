...
void TextEditor::handleMouseInteractions()
{
	...
	else if (ImGui::IsWindowHovered())
	{
		...
		if (columnSelecting && ImGui::IsMouseDragging(ImGuiMouseButton_Left))
		{
			...
			// Build the column cursors. Lines whose maxColumn is less than the
			// requested LEFT edge are dropped (no cursor at all) — clamping
			// them to maxColumn caused weird drift / merged cursors / crashes
			// once Cursors::update tried to merge them.
			bool first = true;
			for (int ln = loLine; ln <= hiLine; ++ln)
			{
				...
				if (selLo > maxCol) continue;            // line too short for this column
				int hi = std::min(selHi, maxCol);        // right edge can clip — that's fine
				int lo = selLo;
				if (lo == hi && selLo != selHi) continue; // zero-width on this line
				Coordinate a(ln, lo), b(ln, hi);
