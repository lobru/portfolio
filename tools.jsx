...
function DeployTool({ data, companion }) {
  ...
  const onRun = async () => {
    setRunStatus("working"); setRunOutput("");
    const script = `echo "=== Deploy checklist · ${data.meta.project} v${data.meta.version} ==="\necho "Companion confirms: build environment reachable."\ncmake --version 2>&1 || echo "cmake not in PATH"`;
